...
function create_theme_from_template(string $themes_root, string $theme_folder, array $template_files): array {
    ...
    foreach ($template_files as $relative_path => $content) {
        $destination = $target_dir . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $relative_path);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($new_theme_name === '') {
        ...
        $error = 'Název smí obsahovat pouze malá písmena, čísla, pomlčku a podtržítko.';
...
<head>
    ...
    <style>
        ...
        .wrap { max-width: 900px; margin: 0 auto; background: #fff; border-radius: 10px; padding: 24px; box-shadow: 0 10px 25px rgba(0,0,0,0.08); }
        h1 { margin-top: 0; }
...
<body>
    <div class="wrap">
        ...
        <p class="hint">Vytvořte novou šablonu (layout) bez ručního kopírování souborů. Poté ji můžete nastavit jako aktivní v <code>config/config.php</code> přes konstantu <code>ACTIVE_THEME</code>.</p>
